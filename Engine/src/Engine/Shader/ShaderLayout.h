...
namespace LM
{
    ...
    struct ShaderAttribute
    {
        ...
        ShaderAttribute(ShaderDataType _Type, std::string_view _Name) : Name(_Name), Type(_Type), Location(0) { }
    ...
    public:
        ...
        ShaderSource(Type _Type, std::pair<std::string, LoadType> _Source)
            : m_Type(_Type),
              m_LoadType(_Source.second),
              m_Source(_Source.first)
        { }
    ...
    public:
        static std::pair<std::string, LoadType> FromSource(std::string_view _Source)
        {
            return { _Source.data(), LoadType::SOURCE };
        }
        static std::pair<std::string, LoadType> FromFile(std::string_view _File)
        {
            return { _File.data(), LoadType::FILEPATH };
        }
